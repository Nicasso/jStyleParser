...
/**
 * Encapsulates one selector for CSS declaration. CombinedSelector can contain
 * classes, attributes, ids, pseudo attributes, and element name, together with
 * combinator according to next placed selectors
 * 
 * @author kapy
...
public class SelectorImpl extends AbstractRule<Selector.SelectorPart> implements Selector {
		...
		for (SelectorPart item : list) {
			if (item instanceof PseudoPage) {
				...
				if (ret.isPseudoElement()) {
					return ret; // pseudo-elements may only be appended after
								// the last simple selector of the selector
				}
			}
		}
		return null;
	...
	public boolean matches(Element e) {
		...
		for (SelectorPart item : list) {
			if (item == null || !item.matches(e, CSSFactory.getElementMatcher(), CSSFactory.getDefaultMatchCondition())) // null
																															// in
																															// case
																															// of
																															// syntax
	...
	public boolean matches(Element e, ElementMatcher matcher, MatchCondition cond) {
		...
		// check other items of simple selector
		for (SelectorPart item : list) {
			if (item == null || !item.matches(e, matcher, cond)) // null in case
																	// of syntax
																	// error
																	// (missing
																	// term)
	...
	public static class PseudoPageImpl implements PseudoPage {
		...
		// decoded element index for nth-XXXX properties -- values a and b in
		// the an+b specification
		...
		public boolean matches(Element e, ElementMatcher matcher, MatchCondition cond) {
			...
			if (declaration != null) { // null declaration means some unknown or
										// unimplemented pseudo
				switch (declaration) {
				case FIRST_CHILD:
				case LAST_CHILD:
				case ONLY_CHILD:
				...
				case EMPTY:
					...
					for (int i = 0; i < elist.getLength(); i++) {
						...
						if (t == Node.ELEMENT_NODE || t == Node.TEXT_NODE || t == Node.CDATA_SECTION_NODE
								|| t == Node.ENTITY_REFERENCE_NODE)
							return false;
					}
					return true;
				default:
					// match all pseudo elements and the pseudo classes
					// specified by an additional condition (usually used for
					// using LINK pseudo class for links)
		...
		/**
		 * Checks whether the element position matches a <code>an+b</code> index
		 * specification.
		 * 
		 * @param pos
		 *            The element position according to some counting criteria.
		 * @param n
		 *            The index specifiaction <code>an+b</code> - <code>a</code>
		 *            and <code>b</code> values in array int[2].
		...
		/**
		 * Computes the count of element siblings before the given element in
		 * the DOM tree.
		 * 
		 * @param e
		 *            The element to be examined
		 * @param sameType
		 *            when set to <code>true</code> only the element with the
		 *            same type are considered. Otherwise, all elements are
		 *            considered.
		...
		/**
		 * Computes the count of element siblings after the given element in the
		 * DOM tree.
		 * 
		 * @param e
		 *            The element to be examined
		...
		private void decodeValue() {
			...
			if (declaration == PseudoDeclaration.NTH_CHILD || declaration == PseudoDeclaration.NTH_LAST_CHILD
					|| declaration == PseudoDeclaration.NTH_OF_TYPE
					|| declaration == PseudoDeclaration.NTH_LAST_OF_TYPE) {
				try {
	...
	public static class ElementDOMImpl implements ElementDOM {
		...
		/**
		 * When set to true, the selector has a maximal specificity (inline).
		 * Otherwise, it has a minimal specificity.
		 */
